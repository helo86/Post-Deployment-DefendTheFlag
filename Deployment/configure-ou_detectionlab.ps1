...
if (!([ADSI]::Exists("LDAP://OU=LocalAdminAccounts,OU=AdministrativeAccounts,DC=windomain,DC=local")))
{
	New-ADOrganizationalUnit -Name "LocalAdminAccounts"  -Path "OU=AdministrativeAccounts,DC=windomain, DC=local"
}else
{

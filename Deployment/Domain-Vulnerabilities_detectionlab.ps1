...
if (!([ADSI]::Exists("LDAP://CN=NewAdmin,OU=ServiceAccounts,OU=AdministrativeAccounts,DC=windomain,DC=local")))
{
...
if (!([ADSI]::Exists("LDAP://CN=LocalAdminPC1,OU=LocalAdminAccounts,OU=AdministrativeAccounts,DC=windomain,DC=local")))
{
	New-ADUser -Name "LocalAdminPC1" -DisplayName "LocalAdminPC1" -SamAccountName "localadminpc1" -UserPrincipalName "localadminpc1" -GivenName "localadminpc1" -Surname "Administrator" -AccountPassword ((ConvertTo-SecureString "TestPassword123!" -AsPlainText -Force)) -Enabled $true -Path "OU=LocalAdminAccounts, OU=AdministrativeAccounts, DC=windomain, DC=local" -ChangePasswordAtLogon $false -PasswordNeverExpires $true
...
if (!([ADSI]::Exists("LDAP://CN=LocalAdminPC2,OU=LocalAdminAccounts,OU=AdministrativeAccounts,DC=windomain,DC=local")))
{
	New-ADUser -Name "LocalAdminPC2" -DisplayName "LocalAdminPC2" -SamAccountName "localadminpc2" -UserPrincipalName "localadminpc2" -GivenName "localadminpc2" -Surname "Administrator" -AccountPassword ((ConvertTo-SecureString "TestPassword123!" -AsPlainText -Force)) -Enabled $true -Path "OU=LocalAdminAccounts, OU=AdministrativeAccounts, DC=windomain, DC=local" -ChangePasswordAtLogon $false -PasswordNeverExpires $true
...
Invoke-Command -ScriptBlock{net localgroup "Remote Desktop Users" /add windomain\localadminpc1} -computername AdminPc.windomain.local
Invoke-Command -ScriptBlock{net localgroup "Administrators" /add windomain\localadminpc1} -computername AdminPc.windomain.local

Invoke-Command -ScriptBlock{net localgroup "Remote Desktop Users" /add windomain\localadminpc2} -computername AdminPc.windomain.local
Invoke-Command -ScriptBlock{net localgroup "Administrators" /add windomain\localadminpc2} -computername AdminPc.windomain.local

Invoke-Command -ScriptBlock{net localgroup "Remote Desktop Users" /add windomain\localadminpc2} -computername AdminPc2.windomain.local
Invoke-Command -ScriptBlock{net localgroup "Administrators" /add windomain\localadminpc2} -computername AdminPc2.windomain.local
...
$Cred = New-Object System.Management.Automation.PSCredential('windomain\localadminpc2', $SecPassword)
...
Invoke-Mimikatz -Command '"kerberos::ptt [0;48b991]-2-0-60a10000-localadminpc2@krbtgt-windomain.local.kirbi"' 
...
if (!([ADSI]::Exists("LDAP://CN=TimeAdmin,OU=ServiceAccounts,OU=AdministrativeAccounts,DC=windomain,DC=local")))
{
...
Set-ADUser -Identity timeadm -Add @{'msDS-AllowedToDelegateTo'=@('TIME/AdminPc2.windomain.local')}
Invoke-Command -ScriptBlock{net localgroup "Remote Desktop Users" /add windomain\timeadm} -computername AdminPc.windomain.local
Invoke-Command -ScriptBlock{net localgroup "Administrators" /add windomain\timeadm} -computername AdminPc.windomain.local

$SecPassword = ConvertTo-SecureString 'x!3dsfds945jjlkJ2mN4QQ2' -AsPlainText -Force
$Cred = New-Object System.Management.Automation.PSCredential('windomain\timeadm', $SecPassword)
$s = New-PSSession -ComputerName AdminPc -Credential $Cred
Invoke-Command –Session $s -ScriptBlock {whoami; hostname}
